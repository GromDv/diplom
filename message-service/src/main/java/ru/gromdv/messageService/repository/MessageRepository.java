...
public interface MessageRepository extends JpaRepository<Message, Long> {
    ...
    @Query(value = "select m.id, m.task_id, m.user_id, m.parent_mess_id, m.title, m.text, m.status, m.date_create, u.username from messages m left join users u on u.id = m.user_id where task_id = ?1", nativeQuery = true)
